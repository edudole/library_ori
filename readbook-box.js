(() => {
  ...
  const EXEC_CACHE_KEY = 'LP360:LIBRARY:SITE_FAST:readbook-exec-d26-v1';
  const EXEC_CACHE_AGE = 10 * 60 * 1000;
  const CATALOG_CACHE_KEY = 'LP360:LIBRARY:SITE_FAST:readbook-catalog-v1';
  ...
  async function resolveExec(forceFresh) {
    ...
    if (!MAIN_API_URL) throw new Error('ไม่พบ URL Apps Script หลักของเว็บไซต์ห้องสมุด');
